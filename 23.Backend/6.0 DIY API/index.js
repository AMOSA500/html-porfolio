...
app.get("/jokes/:id", (req, res) => {
  ...
  const getJoke = jokes.find((joke) => joke.id === id);
  res.send(getJoke);
});
//3. GET a jokes by filtering on the joke type
app.get("/filtered/jokes?", (req, res) => {
  const jokeType = req.query.type;
  const filteredActivities = jokes.filter((joke) => joke.jokeType === jokeType);
...
//5. PUT a joke
app.put("/update/joke/:id", (req, res) => {
...
//6. PATCH a joke
app.patch("/patch/joke/:id", (req, res) => {
  const jokeId = parseInt(req.params.id);
...
app.delete("/delete/joke/:id", (req, res) => {
  ...
  const searchIndex = jokes.findIndex((joke) => joke.id === jokeId);
  if(searchIndex < 0) {
    res.status(404).json({error: `Joke with id ${jokeId} not found. No joke was deleted. Please try again.`});
  }else{
    jokes.splice(searchIndex,1);
    res.status(200).json({message: `Joke with id ${jokeId} was successfully deleted.`});
  }
});
